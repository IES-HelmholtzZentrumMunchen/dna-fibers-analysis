...
class TestAnalysis(unittest.TestCase):
    def setUp(self):
        ...
        self.data = ana.pd.DataFrame({
            ...
            index=ana.pd.Int64Index(
                [0, 0, 0, 1, 1, 1, 2, 3, 4, 4, 4], name='profile'))

        self.fork_rate = [1.049180308930711, 6.8333346578917489]
    ...
    def test_fork_speed(self):
        data = self.get_data()
        ana.np.testing.assert_allclose(ana.fork_speed(data), self.fork_speed)

    def test_fork_rate(self):
        data = self.get_data()
        ana.np.testing.assert_allclose(ana.fork_rate(data), self.fork_rate)
