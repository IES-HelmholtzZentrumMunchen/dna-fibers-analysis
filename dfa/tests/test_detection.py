...
class TestDetection(unittest.TestCase):
    def setUp(self):
        ...
        self.fibers = {
            10: {
                0.5: list(list(zip(*ut.read_fibers(
                    data_path, 'coordinates_a0.5_b0.5_l10')))[0]),
                1.0: list(list(zip(*ut.read_fibers(
                    data_path, 'coordinates_a0.5_b1.0_l10')))[0]),
                2.0: list(list(zip(*ut.read_fibers(
    ...
    def test_fiberness_filter(self):
        for beta in self.fiberness.keys():
            fiberness, directions = det.fiberness_filter(
                self.image, scales=[2, 3, 4], mask=None,
                alpha=0.5, beta=beta, gamma=1)
